...
- **No bespoke embedding HTTP endpoints** — retrieval reuses `referencefile` `show_all` / `download_file` / `conversion_request`. The only new HTTP surface is new query params on `show_all`.

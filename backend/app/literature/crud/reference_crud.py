    ...
    for field, value in reference_update.items():
        if field == "resource":
            ...
            if not merged_into_obj:
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                                    detail=f"Merged_into Reference with curie {value} does not exist")
            reference_db_obj.merged_into = merged_into_obj
        else:
...
def show(db: Session, curie: str):
    ...
    if reference.mergee_references:
        mergees = []
        for mergee in reference.mergee_references:
            mergees.append(mergee.curie)
        reference_data['mergee_reference_curies'] = mergees

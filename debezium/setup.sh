...
# Gate 1 (SCRUM-6231): instead of a blind ${KSQL_SETUP_SLEEP}s wait, poll until the Debezium
# snapshot has created the source topics that the ksql CTAS sources reference. Test mode keeps the
# fixed short sleep so the integration test's timing is unchanged; in production KSQL_SETUP_SLEEP is
# the max-wait cap. expected_count = number of tables in the connector's table.include.list (so we
# wait for all per-table topics, robust to a slow snapshot on a big table); 0 if undeterminable
# (the helper then falls back to topic-set stability).
if [[ "${ENV_STATE}" == "test" ]]; then
...
if [[ "${ENV_STATE}" == "test" ]]; then
    ...
    # DRAINS -- ES index_total plateaus + index_current 0 (activity-based, so it survives the sink's
    # by-id upserts that keep doc COUNT flat while joined objects are still re-indexed) AND both ES
    # sink connectors are RUNNING with no failed tasks. DATA_PROCESSING_SLEEP is the max-wait cap.

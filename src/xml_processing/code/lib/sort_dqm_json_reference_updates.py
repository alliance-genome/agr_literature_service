...
def load_pmids_not_found():
    ...
    if path.isfile(pmids_not_found_file):
        with open(pmids_not_found_file) as read_fh:
            for line in read_fh:
...
def sort_dqm_references(input_path, input_mod):  # noqa: C901
    ...
    xrefs_to_add = {}
    aggregate_mod_reference_types_only = {}
    aggregate_mod_biblio_all = {}
            ...
            for entry in entries:
                    ...
                    for cross_reference in entry["crossReferences"]:
                        if "id" in cross_reference:
                            ...
                            if "pages" in cross_reference:
                                xref_to_pages[cross_reference["id"]] = cross_reference[
                                    "pages"
                                ]
                ...
                if len(agrs_found) == 0:
                    ...
                    fh_mod_report[mod].write("dqm %s too many matches %s\n"
                                             % (entry["primaryId"], ", ".join(sorted(
                                                map(lambda x: url_ref_curie_prefix + x, agrs_found)))))
                elif len(agrs_found) == 1:
...
def update_db_entries(
    headers, entries, live_changes, report_fh, processing_flag
):  # noqa: C901
    """
    ...
    fields_simple_camel = ["title", "allianceCategory", "citation", "volume", "pages", "language", "abstract",
                           "publisher", "issueName", "issueDate", "datePublished", "dateLastModified"]
    ...
    for agr in entries:
        ...
        headers = update_mod_reference_types(
            live_changes, headers, agr, dqm_entry, db_entry
        )
    ...
    for mod in dqm_mrt_data:
        ...
        for dqm_mrt in dqm_mrt_data[mod]:
            ...
            if create_it:
                ...
                new_entry = {"reference_type": dqm_mrt, "source": mod, "reference_curie": agr}
                headers = generic_api_post(live_changes, url, headers, new_entry, agr, None, None)
...
def generic_api_patch(
    live_changes, url, headers, update_json, agr, mapping_fh, error_fh
):
    if live_changes:
...
def generic_api_delete(
    live_changes, url, headers, json_data, agr, mapping_fh, error_fh
):
    if live_changes:

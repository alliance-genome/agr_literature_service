...
def generate_resource_abbreviation_to_nlm_from_dqm_references(input_path, mod):  # noqa: C901
    """
    This mapping via abbreviationSynonyms to mod_to_nlm never matches anything after adding the
     mapping via online and print issn through crossReferences.  Leaving this here in
    case future data finds it helps with MOD data that lacks issn information, but has \
    resource abbreviationSynonyms that matches against reference resourceAbbreviation and PubMed XML nlm.
    This takes about a minute to run, compared to the whole script running in a second without this.

    FB have FB ids for resources, but from the references's resourceAbbreviation and pubmed xml's nlm,
    we can update to match against resources's abbreviationSynonyms
...
def load_mod_resource(json_storage_path, pubmed_by_nlm, nlm_by_issn, mod):  # noqa: C901
    ...
    resource_fields_not_in_pubmed = ["titleSynonyms", "abbreviationSynonyms", "copyrightDate", "publisher",
                                     "editorsOrAuthors", "volumes", "pages", "abstractOrSummary"]
    base_path = environ.get("XML_PATH")

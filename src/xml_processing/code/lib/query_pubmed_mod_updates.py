...
def query_pubmed_mod_updates(search_path, search_outfile_path):
    """
    ...
    query_mods(search_path, search_outfile_path)  # query pubmed for mod references


def query_mods(search_path, search_outfile_path):
    """
    ...
    for mod in mods_to_query:
        ...
        if re.findall(r"<Id>(\d+)</Id>", xml_all):
            ...
            for pmid in pmid_group:
                ...
                # new_pmids.append(pmid)
            logger.info(
                "%s search pmids not in alliance count : %s", mod, len(new_pmids)
            )
            search_output += (
                mod
                + " search pmids not in alliance count : "
                + str(len(new_pmids))
                + "\n"
            )
...
def query_pmc_mgi():
    ...
    terms = [
        '("Aging+cell"[Journal])+AND+(mice)',
        '("Cell death and differentiation"[Journal])+AND+(mice)',
        '("Cilia"[Journal])+AND+(mice)',
        '("Disease models mechanisms"[Journal])+AND+(mice)',
        '("Journal of lipid research"[Journal])+AND+(mice)',
        '("Nucleic acids research"[Journal])+AND+(mice)',
        '("PLoS computational biology"[Journal])+AND+(mice)',
        '("PLoS genetics"[Journal])+AND+(mice)',
        '("PloS one"[Journal])+AND+(mice)',
    ]
    ...
    for term in terms:
        ...
        if re.findall(r"<Id>(\d+)</Id>", pmc_xml_all):
            ...
            for index in range(0, len(pmc_group), idconv_slice_size):
                ...
                url = (
                    "https://www.ncbi.nlm.nih.gov/pmc/utils/idconv/v1.0/?tool=my_tool&idtype=pmcid&ids="
                    + pmcs_joined
                )
                # print(url)
                f = urllib.request.urlopen(url)
...
def download_pmc_without_pmid_mgi():
    ...
    sleep_delay = 1
    articleTypes = {"research-article", "review-article", "other", "correction", "editorial",
                    "article-commentary", "brief-report", "case-report", "letter", "discussion",
                    "retraction", "oration", "reply", "news", "expression-of-concern"}
...
if __name__ == "__main__":
    ...
    query_pubmed_mod_updates(search_path, search_outfile_path)

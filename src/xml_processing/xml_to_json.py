        ...
        with open(filename) as xml_file:
            ...
            if title_re_output is not None:
                ...
                if book_title_re_output is not None:
                    ...
                    if vernacular_title_re_output is not None:
                        # print title
                        title = (
                            vernacular_title_re_output.group(1)
                            .replace("\n", " ")
                            .replace("\r", "")
                        )
                        title = re.sub(r"\s+", " ", title)
            ...
            if len(comments_corrections_group) > 0:
                ...
                for comcor_xml in comments_corrections_group:
                    ...
                    if (
                        (other_pmid != "")
                        and (ref_type != "")
                        and (ref_type != "CommentIn")
                        and (ref_type != "CommentOn")
                    ):
                        if ref_type in data_dict["commentsCorrections"]:
                            if (
                                other_pmid
                                not in data_dict["commentsCorrections"][ref_type]
                            ):
                                data_dict["commentsCorrections"][ref_type].append(
                                    other_pmid
                                )
                        else:
            ...
            if len(authors_group) > 0:
                ...
                for author_xml in authors_group:
                    ...
                    # </AffiliationInfo>
                    affiliation_list = []
                    affiliation_info_group = re.findall(
            ...
            date_received_re_output = re.search(
            ...
            article_id_list_re_output = re.search(
                "<ArticleIdList>(.*?)</ArticleIdList>", xml, re.DOTALL
            )
            if article_id_list_re_output is not None:
                article_id_list = article_id_list_re_output.group(1)
                # print pmid + " AIDL " + article_id_list
                article_id_group = re.findall(
                    '<ArticleId IdType="(.*?)">(.+?)</ArticleId>', article_id_list
                )
                    ...
                    for type_value in article_id_group:
                        ...
                        if type in known_article_id_types:
                            ...
                            cross_references.append(
                                {"id": known_article_id_types[type]["prefix"] + value}
                            )
                            data_dict[
                                type
                            ] = value  # for cleaning up crossReferences when reading dqm data
                        else:
            ...
            medline_journal_info_re_output = re.search(
                "<MedlineJournalInfo>(.*?)</MedlineJournalInfo>", xml, re.DOTALL
            )
            if medline_journal_info_re_output is not None:
                ...
                issn_re_output = re.search(
                    "<ISSNLinking>(.+?)</ISSNLinking>", medline_journal_info
                )
                ...
                journal_abbrev_re_output = re.search(
                    "<MedlineTA>(.+?)</MedlineTA>", medline_journal_info
                )
            ...
            if len(regex_abstract_output) > 0:
                for abs in regex_abstract_output:
                    regex_abstract_text_output = re.findall(
                        "<AbstractText.*?>(.+?)</AbstractText>", abs, re.DOTALL
                    )
            ...
            plain_abstract_list = []
            lang_abstract_list = []
            regex_other_abstract_output = re.findall(
                "<OtherAbstract (.+?)</OtherAbstract>", xml, re.DOTALL
            )
            ...
            pip_abstract = " ".join(
                pip_abstract_list
            )  # e.g. 9643811 has pip but not main
            if pip_abstract != "":
            ...
            regex_keyword_output = re.findall(
                "<Keyword .*?>(.+?)</Keyword>", xml, re.DOTALL
            )
            ...
            if len(meshs_group) > 0:
                ...
                for mesh_xml in meshs_group:
                    descriptor_re_output = re.search(
                        "<DescriptorName.*?>(.+?)</DescriptorName>", mesh_xml, re.DOTALL
                    )
                    if descriptor_re_output is not None:
                        mesh_heading_term = descriptor_re_output.group(1)
                        qualifier_group = re.findall(

...
def _build_resource_permission_metadata(
    ...
    if resource_image_permission and resource_image_permission.image_permission:
        ...
        return {
            ...
            "permission_doc_url": image_permission.permission_doc_url,
...
def patch(db: Session, curie_or_reference_id: str, reference_update) -> dict:
    ...
    # Store the old retraction_status before any changes are made
    old_retraction_status = reference_db_obj.retraction_status
...
def delete_reference_email(db: Session, curie_or_reference_id: str, reference_email_id: int):
    ...
    row = (
        ...
        .filter(
            ReferenceEmailModel.reference_id == ref_id,
            ReferenceEmailModel.reference_email_id == reference_email_id,
        )
        .first()
    )
    ...
    if row is None:
        raise HTTPException(
            ...
            detail=f"Reference email link {reference_email_id} not found for this reference"
        )

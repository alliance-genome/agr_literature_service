...
def search_references(query: str = None, facets_values: Dict[str, List[str]] = None,
    ...
    res = es.search(index=config.ELASTICSEARCH_INDEX, body=es_body)
    return {
        "hits": [{"curie": ref["_source"]["curie"], "title": ref["_source"]["title"], "date_published": ref["_source"]["date_published"], "abstract": ref["_source"]["abstract"], "cross_references": ref["_source"]["cross_references"], "authors": ref["_source"]["authors"], "highlight": ref["highlight"] if "highlight" in ref else "highlight" : []} for ref in res["hits"]["hits"]],

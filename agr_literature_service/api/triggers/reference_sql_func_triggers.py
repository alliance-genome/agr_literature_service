...
CREATE OR REPLACE FUNCTION lit.reference_update_citation()
  returns TRIGGER
  language plpgsql
as $$
BEGIN
  raise notice 'reference_update_citation: % % : % %', NEW.reference_id, NEW.title, OLD.title, OLD.curie;
  call lit.update_citations(NEW.reference_id);
...
# DROP TRIGGER IF EXISTS lit.reference_citation_trigger on public.reference;
# CREATE TRIGGER lit.reference_citation_trigger
# AFTER UPDATE OR INSERT ON lit.reference
#        FOR EACH ROW
#        EXECUTE FUNCTION lit.reference_update_citation();
# """

reference_citation_insert_trigger = """
DROP TRIGGER IF EXISTS reference_citation_insert_trigger on lit.reference;
CREATE TRIGGER reference_citation_insert_trigger
AFTER INSERT ON lit.reference
    FOR EACH ROW
    EXECUTE FUNCTION lit.reference_update_citation();
"""

reference_citation_update_trigger = """
DROP TRIGGER IF EXISTS reference_citation_update_trigger on lit.reference;
CREATE TRIGGER reference_citation_update_trigger
AFTER UPDATE ON lit.reference
    FOR EACH ROW
    WHEN (

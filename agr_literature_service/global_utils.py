...
def get_next_local_curie(subdomain, db):
    ...
    if subdomain == 'reference':
        ...
        rs = db.execute(text("SELECT curie FROM lit.reference order by reference_id desc limit 1"))
    else:
        rs = db.execute(text("SELECT curie FROM lit.resource order by resource_id desc limit 1"))
    rows = None
    if rs:
        rows = rs.fetchall()

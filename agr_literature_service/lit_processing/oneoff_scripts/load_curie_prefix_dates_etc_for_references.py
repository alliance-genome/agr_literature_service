...
def load_data():
    ...
    for index in range(loop_count):
        ...
        offset = index * limit
        rs = db_session.execute("SELECT reference_id, date_created, created_by FROM lit.reference order by reference_id limit " + str(limit) + " offset " + str(offset))
        rows = rs.fetchall()
        if len(rows) == 0:
            break
    ...
    for index in range(loop_count):
        offset = index * limit
        rs = db_session.execute("SELECT curie, reference_id, curie_prefix FROM lit.cross_reference WHERE reference_id is not null order by reference_id limit " + str(limit) + " offset " + str(offset))
        rows = rs.fetchall()
        if len(rows) == 0:
            break
        for x in rows:

...
#     if not result:
#         result = parse_year_dash(date_string)
    if not result:
...
def parse_year_space_mon_dash_year_space_mon(date_string):
    ...
    re_output = re.search(r"^(\d{4}) ([A-Za-z]{3})-(\d{4}) ([A-Za-z]{3})$", date_string, re.DOTALL)
...
def parse_year_space_mon_space_day_dash_year_space_mon_space_day(date_string):
    ...
    re_output = re.search(r"^(\d{4}) ([A-Za-z]{3}) (\d{1,2})-(\d{4}) ([A-Za-z]{3}) (\d{1,2})$", date_string, re.DOTALL)
    if re_output is not None:
        year1 = re_output.group(1)
        month1 = month_name_to_number_string(re_output.group(2))
...
def parse_year_space_mon_dash_mon_space_day(date_string):
    ...
    re_output = re.search(r"^(\d{4}) ([A-Za-z]{3})-([A-Za-z]{3}) (\d{1,2})$", date_string, re.DOTALL)
    if re_output is not None:
...
def parse_year_space_mon_space_day(date_string):
    ...
    re_output = re.search(r"^(\d{4}) ([A-Za-z]{3}) (\d{1,2})$", date_string, re.DOTALL)
    if re_output is not None:
        year = re_output.group(1)
...
def parse_year_space_mon(date_string):
    ...
    re_output = re.search(r"^(\d{4}) ([A-Za-z]{3})$", date_string, re.DOTALL)
    if re_output is not None:
        year = re_output.group(1)
        month = month_name_to_number_string(re_output.group(2))
...
def parse_year_space_mon_space_day_dash_mon_space_day(date_string):
    # year space three letter month abbreviation space day dash three letter month abbreviation space day
    re_output = re.search(r"^(\d{4}) ([A-Za-z]{3}) (\d{1,2})-([A-Za-z]{3}) (\d{1,2})$", date_string, re.DOTALL)
    if re_output is not None:
        year = re_output.group(1)
        month1 = month_name_to_number_string(re_output.group(2))
        day1 = re_output.group(3)
...
def parse_year_space_mon_space_day_dash_day(date_string):
    ...
    re_output = re.search(r"^(\d{4}) ([A-Za-z]{3}) (\d{1,2})-(\d{1,2})$", date_string, re.DOTALL)
    if re_output is not None:
        year = re_output.group(1)
        month = month_name_to_number_string(re_output.group(2))
        day1 = re_output.group(3)
...
def parse_year_space_mon_dash_mon(date_string):
    ...
    re_output = re.search(r"^(\d{4}) ([A-Za-z]{3})-([A-Za-z]{3})$", date_string, re.DOTALL)
    if re_output is not None:
        year = re_output.group(1)
        month1 = month_name_to_number_string(re_output.group(2))
        month2 = month_name_to_number_string(re_output.group(3))
...
def parse_date_timezone(date_string):
    just_year_re_output = re.search(r"^(\d{4})-(\d{2})-(\d{2})T00", date_string, re.DOTALL)
    if just_year_re_output is not None:
        year = just_year_re_output.group(1)
        month = just_year_re_output.group(1)
        day = just_year_re_output.group(1)
        return f"{year}-{month}-{day}", f"{year}-{month}-{day}"
    return False
...
def parse_year_space_month_dash_day(date_string):
    ...
    re_output = re.search(r"^(\d{4}) (\d{1,2})-(\d{1,2})$", date_string, re.DOTALL)
    if re_output is not None:
        year = re_output.group(1)
...
def parse_year_dot_or_dash_month(date_string):
    re_output = re.search(r"^(\d{4})[-\.](\d{1,2})$", date_string, re.DOTALL)
    if re_output is not None:
        year = re_output.group(1)
        month = re_output.group(2)
        if int(month) > 12:
...
def parse_year_dash_year(date_string):
    re_output = re.search(r"^(\d{4})-(\d{4})$", date_string, re.DOTALL)
    if re_output is not None:
        year1 = re_output.group(1)
...
# def parse_year_dash(date_string):
#     just_year_re_output = re.search(r"^(\d{4})\-$", date_string, re.DOTALL)
#     if just_year_re_output is not None:
#         year = just_year_re_output.group(1)
...
def parse_just_date(date_string):
    just_year_re_output = re.search(r"^(\d{4})-(\d{2})-(\d{2})$", date_string, re.DOTALL)
    if just_year_re_output is not None:
        year = just_year_re_output.group(1)
        month = just_year_re_output.group(1)
        day = just_year_re_output.group(1)
...
for row in rows:
    ...
    if date_range:
        logger.info(f"{date_string}\t{date_range}\t{curie}")
    else:

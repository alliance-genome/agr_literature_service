...
COPY debezium/tables_aggregator/pom.xml /tmp/tables_aggregator/

# Download all Maven dependencies; unless the POM changes, this step will be cached for future builds
RUN mvn dependency:go-offline -f /tmp/tables_aggregator/pom.xml

RUN mvn dependency:copy-dependencies -f /tmp/tables_aggregator/pom.xml -DoutputDirectory=target/dependencies -DincludeScope=compile
COPY debezium/tables_aggregator/src /tmp/tables_aggregator/src
RUN mvn package -DskipTests -o -f /tmp/tables_aggregator/pom.xml

# 3. Create actual image (jlink-ed JDK, dependencies, JAR and launcher)
FROM registry.fedoraproject.org/fedora-minimal:37

COPY --from=jdk /opt/jre-minimal /opt/tables_aggregator/jdk
RUN cd /opt/tables_aggregator \
...
COPY --from=build /tmp/tables_aggregator/target/dependencies/* /opt/tables_aggregator/lib/
COPY debezium/tables_aggregator/run-aggregator.sh /opt/tables_aggregator
COPY --from=build /tmp/tables_aggregator/target/tables_aggregator-0.1-SNAPSHOT.jar /opt/tables_aggregator/lib/tables_aggregator-0.1-SNAPSHOT.jar

# ENTRYPOINT /opt/tables_aggregator/run-aggregator.sh
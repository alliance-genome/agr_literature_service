...
class MockDataFactory:
    ...
    def create_reference(self, db_session, ref_id: int, citation: CitationModel, 
        ...
        ref_pattern = (self.mock_patterns.get('references', [{}])[ref_id % len(self.mock_patterns.get('references', [{}]))]
                      if self.mock_patterns.get('references') else {})
    ...
    def create_author(self, db_session, reference: ReferenceModel, author_id: int) -> AuthorModel:
        ...
        author_pattern = (self.mock_patterns.get('authors', [{}])[author_id % len(self.mock_patterns.get('authors', [{}]))]
                         if self.mock_patterns.get('authors') else {})
    ...
    def create_copyright_license(self, db_session, license_id: int) -> CopyrightLicenseModel:
        ...
        license_pattern = (self.mock_patterns.get('copyright_licenses', [{}])[license_id % len(self.mock_patterns.get('copyright_licenses', [{}]))]
                          if self.mock_patterns.get('copyright_licenses') else {})
...
class TestDebeziumIntegration:
    ...
    def test_public_index_limited_fields(self, elasticsearch_config):
        ...
        if data['hits']['total']['value'] > 0:
            ...
            # Required fields for public index
            required_fields = {
                'curie', 'title', 'cross_references', 'relations', 'authors', 
                'copyright_license', 'citation', 'abstract', 'category', 'pubmed_types',
                'resource_title', 'volume', 'issue_name', 'page_range', 'publisher', 
                'language', 'date_published', 'pubmed_publication_status',
                'date_arrived_in_pubmed', 'date_last_modified_in_pubmed', 
                'date_created', 'keywords', 'mesh_terms'
            }
    ...
    def test_private_index_full_fields(self, elasticsearch_config):
        ...
        if data['hits']['total']['value'] > 0:
            ...
            # Check for MOD-specific fields that should be in private only
            sensitive_fields = {'mods_in_corpus', 'workflow_tags', 'topic_entity_tags'}
            # At least some sensitive fields should be present (if data exists)
            # This is optional since test data might not have all fields

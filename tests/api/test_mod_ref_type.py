...
from agr_literature_service.api.models import ReferenceModel, ReferenceCommentAndCorrectionModel, ModReferenceTypeModel
...
class TestModReferenceType:
    ...
    def test_create_mrt(self, db, create_test_mod_ref_type):
    ...
    def test_show_mrt(self, create_test_mod_ref_type):
        with TestClient(app) as client:
            response = client.get(url=f"/reference/mod_reference_type/{create_test_mod_ref_type[0].json()}")
            assert response.status_code == status.HTTP_200_OK

    def test_destroy_mrt(self, create_test_mod_ref_type, auth_headers):
        with TestClient(app) as client:
